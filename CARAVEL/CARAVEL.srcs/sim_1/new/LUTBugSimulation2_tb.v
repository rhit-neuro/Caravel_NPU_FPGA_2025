...
module LUTBugSimulation2_tb();
  ...
  //------------------------------------------------------------------------------
  // ZipDMA integration (REAL DMA master)
  //
  // Existing testcase_dma_then_cpu() emulates WB4 traffic by directly driving
  // wb4_* signals from the TB.
  //
  // The additions below instantiate the actual zipdma.v core, plus a small
  // Wishbone RAM model used as the DMA source.  A simple address decoder
  ...
  always @(posedge clock) begin
    if (reset) begin
      ...
      // One-cycle, single-pulse ACK per request.
      // - Capture a request into ram_pending (when not already pending)
      // - Assert ACK for one cycle on the following clock
      ram_ack     <= ram_pending;
      ram_pending <= (ram_req && !ram_pending);

      if (ram_req && !ram_pending) begin
        if (!dma_mwb_we) begin
          ram_rdata <= dma_src_mem[ram_word_index];
        end else begin
  ...
  task zipdma_copy;
    ...
    begin
      ...
      dma_ctrl_write(2'b00, 32'h6000_0000);
      dma_wait_done();
    end
  endtask
  ...
  task testcase_zipdma_then_cpu;
    ...
    begin
      load_tables_zipdma();
      snapshot_pre();
      dump_tables_to_file("zipdma-pre-calc.csv","zipdma-pre", V_PRE, M_PRE, B_PRE);
      compute_f1_wb3(32'h40A00000, y); // x = 5.0 (example)
      snapshot_post();
      dump_tables_to_file("zipdma-post-calc.csv","zipdma-post", V_POST, M_POST, B_POST);
