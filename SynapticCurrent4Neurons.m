...
tau_rise = zeros(N,1, 'single');
tau_decay =  zeros(N,1, 'single');
g_syn_bar = zeros(N,1, 'single');
E_syn =  zeros(N,1, 'single');
t_peak = zeros(N,1, 'single');
f =  zeros(N,1, 'single');
h_0 = zeros(N,1, 'single');
g_0 =  zeros(N,1, 'single');

for i = 1:N 
    tau_rise(i) = single(2.5);%0x40200000
    tau_decay(i) = single(11); %0x41300000
    g_syn_bar(i) = single(200e-9);%0x3456bf95
    E_syn(i) = single(0.045);  % using values for sodium  % 0x3d3851ec
    t_peak(i) = single(t0 + ((tau_decay*tau_rise)/(tau_decay*tau_rise))*log(tau_decay/tau_rise)); %0x3fbda512
    f(i) = single(1/(-exp((-(t_peak-t0)/tau_rise)+exp(-(t_peak-t0)/tau_decay)))); %0xbf413a93
...
for i = 2:loop
    ...
    % Calculate I syn
    I_syn(i) = single(g_syn(i) * (Vmem(i) - E_syn));
